    ...
    public class User
    {
      ...
      /*  void AddCategory(string newCategory)
        ...
        public void SaveUserInfoToDatabase()
        {
            ...
            sql = "INSERT OR REPLACE INTO userTable  (username, password, name, cprNumber, phone, email, tag) values ('" + user.UserName + "', '" + user.Password + "', '" + user.Name + "' , " + user.CprNumber + " , " + user.Phone + ", '" + user.Email + "', '" + Database.Instance.listToString(user.UserCategories) +"')";
            Database.Instance.Execute(sql);
        }
